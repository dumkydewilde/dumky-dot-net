...
variable "dbt_repo_url" {
  description = "A URL pointing to the dbt project to use. "
  default     = "https://github.com/dumkydewilde/snowplow-serverless.git" 
  type        = string
}

variable "dbt_repo_folder_name" {
  description = "dbt project folder inside the repository"
  default     = "snowplow-serverless/dbt"
  type        = string
}
